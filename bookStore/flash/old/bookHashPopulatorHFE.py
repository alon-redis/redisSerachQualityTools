...
def apply_field_expirations(r, key, field_names, expiration_range):
    ...
    for field in field_names:
        ...
        pipe.hexpire(key, ttl_seconds, field)
    pipe.execute()
...
def generating_books(connection_pool, max_books, max_random, expiration_range=None):
    try:
        ...
        for _ in range(1, max_books + 1):
            ...
            apply_field_expirations(r, key, flat.keys(), expiration_range)
...
if __name__ == "__main__":
    arg_parser = argparse.ArgumentParser(description="Running the book store application v2.1 (Flex/disk index compatible: no SORTABLE, no NUMERIC, no GEO fields)")
    ...
    arg_parser.add_argument(
        ...
        help="Optional random hash field expiration in seconds, format X-Y (e.g., 60-300). "
             "When set, each hash field gets a random TTL in [X, Y] via HEXPIRE. "
    ...
    try:
        ...
        if args.expiration_range is not None:
            ...
            print(
                f"Hash field expiration enabled: each field gets a random TTL between {x} and {y} seconds "
                f"(verification doc at {make_key(0)} is exempt and never expires)."
            )
        else:
            print("Hash field expiration disabled (no TTL on fields).")
